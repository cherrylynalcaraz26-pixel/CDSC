...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Marking the quotation confirmed fires a DB trigger (create_so_from_accepted_quotation)
  // that creates the Sales Order + so_items and makes it visible in the client portal —
  // the same trigger also fires when a quotation is accepted from the admin panel or the
  // portal itself, so every acceptance path produces exactly one order.
